...
public class TrashSpawnManager : MonoBehaviour
{
    ...
    private IEnumerator Start()
    {
        ...
        {
            yield return null; // 플레이어 잡힐때까지 프레임 넘김
            this.player = FindObjectOfType<PlayerCellMove>(); // 플레이어 셀 이동 스크립트 가져오기
        }
    ...
    {
        GetTerrain();
        DataManager.Instance.dicTrash.Clear(); // 기존 데이터를 클리어
    ...
    public void TrashRecreate()
    {
        this.player.CellMoveAction(new Vector2Int(999, 999)); // 간단하게 화면에 있는 쓰레기를 리로드 하기 위해 존재하지 않는 셀에 플레이어 잠깐 넣었다가
                                                              // 다시 본래 위치로 빼옴
        CreateTrashData(200); // 쓰레기 데이터를 생성
        DataManager.Instance.SaveTrashData(); // 쓰레기 데이터를 저장
