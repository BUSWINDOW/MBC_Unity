...
public class GridManager : MonoBehaviour
{
    ...
    public Node NodeFromWorldPoint(Vector3 worldPos)
    {
        ...
        // 0,0,0 기준으로 생각하면, gridManager가 0,0,0 , 그리고 찾아야할 좌표도 0,0,0일때, 해당 좌표는 정중앙이다.
        // 그런 느낌으로 그리드상의 좌표를 맞추는것

        int x = Mathf.RoundToInt((gridSizeX - 1) * percentX);
        int y = Mathf.RoundToInt((gridSizeY - 1) * percentY);
        int z = Mathf.RoundToInt((gridSizeZ - 1) * percentZ);
        //그걸 전체개수(배열이니 1빼서)에 곱하고, 그걸 int로 바꿔서 처리
        return grid[x, y, z];
    }
    ...
    int GetDistance(Node a, Node b) //일정한 크기의 그리드로 했으니, 그냥 두 그리드 사이의 거리를 구한다.
    {
        ...
        //return Mathf.RoundToInt(Vector3.Distance(Vector3.zero, new Vector3(dx, dy, dz)));
        return dx + dy + dz;
    }
